...
export function RefundItem({ data, ...rest }: Props) {
  return (
    <a
      className="flex items-center gap-3 hover:gray-green-100/5 cursor-pointer rounded-md p-2"
      {...rest}
    >
      <img
        className="w-8 h-8"
        src={data.categoryImg}
        alt="Ïcone da categoria"
      />
      ...
      <span className="text-sm text-gray-100 font-semibold">
        <small className="fornt-normal text-gray-200">R$</small>
        {data.amount}
      </span>
    </a>
  );
